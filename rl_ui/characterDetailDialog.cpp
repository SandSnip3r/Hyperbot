...
QTimer *CharacterDetailDialog::globalCooldownTimer_ = nullptr;
QList<CharacterDetailDialog *> CharacterDetailDialog::activeDialogs_;
...
CharacterDetailDialog::CharacterDetailDialog(const sro::pk2::GameData &gameData,
  ...
  if (!globalCooldownTimer_) {
    globalCooldownTimer_ = new QTimer;
    globalCooldownTimer_->setInterval(100);
    connect(globalCooldownTimer_, &QTimer::timeout, &CharacterDetailDialog::handleGlobalTimeout);
  }
  if (!globalCooldownTimer_->isActive()) {
    globalCooldownTimer_->start();
  }
  activeDialogs_.append(this);
}

CharacterDetailDialog::~CharacterDetailDialog() {
  activeDialogs_.removeOne(this);
  if (activeDialogs_.isEmpty() && globalCooldownTimer_) {
    globalCooldownTimer_->stop();
    delete globalCooldownTimer_;
    globalCooldownTimer_ = nullptr;
  }
  delete ui_;
}
...
void CharacterDetailDialog::updateCharacterData(const CharacterData &data) {
  ...
  for (const SkillCooldown &cooldown : cooldowns) {
    ...
    const auto &skill = gameData_.skillData().getSkillById(cooldown.skillId);
    const int fullMs = skill.actionReuseDelay;

    QProgressBar *bar = new QProgressBar(container);
    bar->setRange(0, fullMs);
...
void CharacterDetailDialog::handleGlobalTimeout() {
  for (CharacterDetailDialog *dlg : activeDialogs_) {
    dlg->updateCooldownDisplays();
  }
}
...
void CharacterDetailDialog::updateCooldownDisplays() {
  for (CooldownItem &item : cooldownItems_) {
    ...
    item.bar->setRange(0, item.fullMs);
