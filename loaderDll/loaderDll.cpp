...
bool bDoRedirectLogin = true;
bool bDoRedirectWorld = true;
		...
		for (size_t x = 0; x < gDivisionInfo.divisions.size(); ++x)
		{
			for (size_t y = 0; y < gDivisionInfo.divisions[x].gatewayIpAddresses.size(); ++y)
			{
				...
				if (remoteHost)
				{
					...
					routeArray[routeListCount].srcPort = 15779; // Just hardcode for now rather than doing another GATEPORT access
					routeListCount++;
				}
				else
				{
...
void UserOnUninject()
{
	if (bDoRedirectLogin || bDoRedirectWorld)
	{
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
...
#undef MakePtr
	...
	// Proxy redirect
	if (bDoRedirectWorld || bDoRedirectLogin)
	{
