...
- If the user needs to send a packet on a channel, this is easily done by `PacketBuilder` objects. Inheriting objects of `PacketBuilder` implement sufficient constructors for each of the valid packet configurations. Then, calling `packet()` on one of these inheriting objects, a complete `PacketContainer` is built and returned. Finally, the packet is passed to `BrokerSystem::injectPacket` and after many levels of abstraction, it's essentially pushed into the outgoing queue of tcp messages to be sent to either the client or the server.
