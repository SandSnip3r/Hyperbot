...
bool PK2Reader::Open(const char * filename) {
	...
	m_root_offset = shared_io::file_tell(m_file);
...
bool PK2Reader::GetEntries(PK2Entry & parent, std::list<PK2Entry> & entries) {
	...
	if (shared_io::file_seek(m_file, parent.position, SEEK_SET) != 0) {
		m_error.str(""); m_error << "Invalid seek index.";
		return false;
	}
	...
	while (true) {
		...
		if (block.entries[19].nextChain) {
			...
			if (shared_io::file_seek(m_file, block.entries[19].nextChain, SEEK_SET) != 0) {
...
bool PK2Reader::GetEntry(const char * pathname, PK2Entry & entry) {
	...
	if (entry.position == 0) {
		if (shared_io::file_seek(m_file, m_root_offset, SEEK_SET) != 0) {
			m_error.str(""); m_error << "Invalid seek index.";
			return false;
		}
	} else {
		if (shared_io::file_seek(m_file, entry.position, SEEK_SET) != 0) {
			m_error.str(""); m_error << "Invalid seek index.";
			return false;
		}
	}
		...
		for (int x = 0; x < 20; ++x) {
			...
			if (name == path) {
				...
				if (tokens.empty()) {
					...
					// bugs could result.
					if (e.type == 1) {
						if (shared_io::file_seek(m_file, e.position, SEEK_SET) != 0) {
							m_error.str(""); m_error << "Invalid seek index.";
							return false;
						}
						cycle = true;
