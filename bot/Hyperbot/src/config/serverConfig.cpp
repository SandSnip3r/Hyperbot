...
void ServerConfig::initialize(const std::filesystem::path &pathToConfig) {
  ...
  proto::server_config::ServerConfig serverConfig;

  // ServerConfig file exists, parse
  bool success;
  if constexpr (kProtobufSavedAsBinary_) {
    // Try to read an existing open config file as binary
    success = serverConfig.ParseFromIstream(&configFileIn);
  } else {
    // Try to read an existing open config file as string
    google::protobuf::io::IstreamInputStream pbIStream(&configFileIn);
    success = google::protobuf::TextFormat::Parse(&pbIStream, &serverConfig);
  }

  if (!success) {
    throw std::runtime_error("ServerConfig file open, but could not parse");
  }
