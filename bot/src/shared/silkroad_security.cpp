...
struct SilkroadSecurityData
{
  ...
  std::list< PacketContainerAndInjected > m_incoming_packets;
...
PacketContainerAndInjected SilkroadSecurity::GetPacketToRecv()
{
  std::unique_lock<std::mutex> incoming_packet_lock(m_data->m_incoming_packet_mutex);
...
void SilkroadSecurity::Recv(const PacketContainer &packet) {
  std::unique_lock<std::mutex> incoming_packet_lock(m_data->m_incoming_packet_mutex);
  m_data->m_incoming_packets.emplace_back(packet, true);
}
