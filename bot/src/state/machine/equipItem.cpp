...
Status EquipItem::onUpdate(const event::Event *event) {
  ...
  CHAR_VLOG(1) << absl::StreamFormat("Found item %s in slot %d", bot_.gameData().getItemName(itemRefId_), *itemSlot);

  // Where does the item go?
  const storage::ItemEquipment *equipment = dynamic_cast<const storage::ItemEquipment*>(bot_.inventory().getItem(*itemSlot));
  if (!equipment) {
