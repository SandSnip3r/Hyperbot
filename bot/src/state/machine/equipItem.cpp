...
Status EquipItem::onUpdate(const event::Event *event) {
  if (!initialized_) {
    ...
    itemSlot_ = *itemSlot;
    LOG(INFO) << absl::StreamFormat("Found item %s in slot %d", bot_.gameData().getItemName(itemRefId_), itemSlot_);
    initialized_ = true;
  }
  ...
  if (childState_) {
    ...
    LOG(INFO) << "Child state finished, we're done.";
    childState_.reset();
    return Status::kDone;
  }
  ...
  LOG(INFO) << absl::StreamFormat("Item %s is an avatar hat, moving from inventory slot %d to avatar slot %d", bot_.gameData().getItemName(itemRefId_), itemSlot_, sro::game_constants::kAvatarHatSlot);
