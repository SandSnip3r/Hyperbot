...
void Alchemy::initialize() {
  ...
  // TODO: Move this to the first onUpdate call.
...
Status Alchemy::onUpdate(const event::Event *event) {
  ...
  if (slotsWithBlade.empty()) {
    ...
    injectPacket(makeItemPacket, PacketContainer::Direction::kBotToServer);
  ...
  if constexpr (kUseLuckStones) {
    ...
    if (!bladeHasLuck) {
      ...
      injectPacket(useStonePacket, PacketContainer::Direction::kBotToServer);
  ...
  injectPacket(useElixirPacket, PacketContainer::Direction::kBotToServer);
  alchemyTimedOutEventId_ = bot_.eventBroker().publishDelayedEvent(event::EventCode::kAlchemyTimedOut, std::chrono::milliseconds(kAlchemyTimedOutMs));
