...
JaxInterface::ActionSelection JaxInterface::selectAction(const model_inputs::ModelInputView &modelInputView,
                                                         bool canSendPacket) {
  ...
  try {
    ...
    py::array_t<float> qValuesNumpy = resultTuple[1].cast<py::array_t<float>>();
    result.qValues.assign(qValuesNumpy.data(), qValuesNumpy.data() + qValuesNumpy.size());
