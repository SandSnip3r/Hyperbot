...
void CharacterInfoModule::abnormalInfoReceived(const packet::parsing::ParsedServerAbnormalInfo &packet) {
  for (int i=0; i<=bitNum(packet_enums::AbnormalStateFlag::kZombie); ++i) {
...
void CharacterInfoModule::useItem(uint8_t slotNum, uint16_t typeData) {
  auto useItemPacket = PacketBuilding::ClientUseItemBuilder(slotNum, typeData).packet();
...
int CharacterInfoModule::getHpPotionDelay() {
  const bool havePanic = (modernStateLevel_[bitNum(packet_enums::AbnormalStateFlag::kPanic)] > 0);
...
int CharacterInfoModule::getMpPotionDelay() {
  const bool haveCombustion = (modernStateLevel_[bitNum(packet_enums::AbnormalStateFlag::kCombustion)] > 0);
  int delay = potionDelayMs_ + kPotionDelayBufferMs_;
  if (haveCombustion) {
    delay += 4000;
  }
...
bool isPill(const pk2::media::Item &itemInfo) {
  return (itemInfo.typeId1 == 3 && itemInfo.typeId2 == 3 && itemInfo.typeId3 == 2);
}

bool isUniversalPill(const pk2::media::Item &itemInfo) {
  return (isPill(itemInfo) && itemInfo.typeId4 == 6);
}

bool isPurificationPill(const pk2::media::Item &itemInfo) {
  return (isPill(itemInfo) && itemInfo.typeId4 == 1);
}

bool isPotion(const pk2::media::Item &itemInfo) {
  return (itemInfo.typeId1 == 3 && itemInfo.typeId2 == 3 && itemInfo.typeId3 == 1);
}

bool isHpPotion(const pk2::media::Item &itemInfo) {
  return (isPotion(itemInfo) && itemInfo.typeId4 == 1);
}

bool isMpPotion(const pk2::media::Item &itemInfo) {
  return (isPotion(itemInfo) && itemInfo.typeId4 == 2);
}

bool isVigorPotion(const pk2::media::Item &itemInfo) {
...
void CharacterInfoModule::serverUseItemReceived(const packet::parsing::ParsedServerUseItem &packet) {
  if (packet.result() == 1) {
    ...
    if (!usedItemQueue_.empty()) {
      ...
      if (packet.errorCode() == packet_enums::InventoryErrorCode::kWaitForReuseDelay) {
