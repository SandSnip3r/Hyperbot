		...
		if(!error)
		{
			if(Silkroad.security)
			{
				while(Silkroad.security->HasPacketToRecv())
				{
					...
					auto newP = p;
					std::cout << "[C->S] Received packet " << newP.opcode << ' ';
					auto size = newP.data.GetReadStreamSize();
					std::string data = newP.data.Read_Ascii(size);
					for (char &c : data) {
						if (c < 32) {
							c = 32;
						}
					...
					std::cout << data << '\n';
					if (p.opcode == 28709) {
						auto data = p.data.GetStreamVector();
						for (auto i : data) {
							printf("%02X ", i);
						}
						std::cout << '\n';
			...
			if(Joymax.security)
			{
				while(Joymax.security->HasPacketToRecv())
				{
					...
					auto newP = p;
					std::cout << "[S->C] Received packet " << newP.opcode << ' ';
					auto size = newP.data.GetReadStreamSize();
					std::string data = newP.data.Read_Ascii(size);
					for (char &c : data) {
						if (c < 32) {
					...
					if (p.opcode == 12326) {
						//[C->S] Received packet 28709     h
						//001 008 001 000 068
						//h
						//[S->C] Received packet 12326    h   h
						//001 0DF 09F 068 000 001 000 068
						//"h"             "h"
						//[C->S] Received packet 28709     hg
						//001 009 002 000 068 067
						//h g
						//[S->C] Received packet 12326    h   hg
						//001 0DF 09F 068 000 002 000 068 067
						//"h"             "h" "g"
						std::vector<uint8_t> data = p.data.GetStreamVector();
						if (data[0] == 0x01) {
							std::vector<uint8_t> uniqueId = std::vector<uint8_t>(data.begin() + 1, data.begin() + 5);
							uint16_t messageSize = (data[5] << 8) | (data[6]);
							std::string message;
							for (int i = 7; i < data.size() && i < 7+messageSize; ++i) {
								message += data.at(i);
							}
							for (char &c : message) {
								if (c > 'A' && c < 'Z') {
							...
							if (message == "deposit") {
								...
								if (!done) {
									...
									//backwards endian
									std::cout << "==========================================================================\n";
									std::cout << "=============================Incoming deposit=============================\n";
									std::cout << "==========================================================================\n";
									printf("==========================Character id: %02X%02X%02X%02X==========================\n", uniqueId[3], uniqueId[2], uniqueId[1], uniqueId[0]);
									std::cout << "======================Dispatching character: AlICaps======================\n";
									std::cout << "==========================================================================\n";
									if (Joymax.security) {
										StreamUtility w;
										for (auto i : uniqueId) {
										...
										Joymax.Inject(0x7081, w);
									}
									std::stringstream ss;
									for (int i = uniqueId.size() - 1; i >= 0; --i) {
										ss << std::hex << (int)data[i];
									}
									std::string idStr = ss.str();
									std::string exepathStr = std::string("\"C:\\Users\\Victor\\Documents\\Development\\AutoIt\\sendTelegram.bat\" \"" + idStr + "\"");
									STARTUPINFO info = { sizeof(info) };
									PROCESS_INFORMATION processInfo;
									//LPCWSTR namePtr = L"Snow";
