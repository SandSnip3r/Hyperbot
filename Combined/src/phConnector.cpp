...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    gameData.parseSilkroadFiles();

    broker::EventBroker eventBroker;
    ui::UserInterface userInterface{gameData, eventBroker};
    Session session{gameData, configData.silkroadDirectory(), configData.characterLoginData(), eventBroker};

    userInterface.initialize();
    session.initialize();

    // TODO: In the future, the world state should live at this level too, then it can be passed into the ctor
